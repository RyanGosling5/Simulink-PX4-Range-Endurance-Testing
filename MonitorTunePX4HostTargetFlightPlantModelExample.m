...
% and plant model and simulate it with the PX4 source code in Software In The Loop (SITL) simulation.
...
%
% * Perform the initial <docid:px4_doccenter#buiyb9j-1> tasks of the 
...
%
% Once the project launches, it loads the required workspace variables
% and opens the top model.
%                                                        
...
%
% *9.* Note that the controller gains are optimized for close tracking of a
% circular trajectory. It may be possible that for other types of input
% commands such as step or ramp, the response is not ideal. In such a
% scenario, the controller gains must be adjusted to meet the specified
% requirements. For more details on the controller tuning, see
...
%
% The uORB Read blocks are used to subscribe to the |vehicle_local_position_groundtruth| and |vehicle_attitude_groundtruth| topics. 
